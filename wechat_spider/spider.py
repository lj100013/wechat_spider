...
class Spider(object):
    ...
    def _get_gzh_url(self,name,retrytimes):
        """
            ...
            try:
                ...
                if len(urls) > 0:
                    ...
                    if 'SNUID=' in cookie_jar:
                        detail_url = self._process_url(urls[0], search_url, cookie_jar, response.text)
    ...
    def _get_article_list(self,gzh_url,name,retrytimes):
        while retrytimes >= 0:
            ...
            # service_args = ["--proxy-type=https", "--proxy=%s:%s" % (self.host, self.port)]
