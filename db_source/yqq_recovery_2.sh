...
do
    ...
    do
        ...
        if [[ ${collection} == '' ]];then
            ...
            mongorestore -h 192.168.3.181:${mydict[$platform]} -u admin -p adminUsfqga --authenticationDatabase admin -v -d ${mongodb} --drop ${dst_path}/${platform}/${mongodb}
            if [ $? == 0 ];then
            ...
            mongorestore -h 192.168.3.181:${mydict[$platform]} -u admin -p adminUsfqga --authenticationDatabase admin -v -d ${mongodb}  -c ${collection} --drop ${dst_path}/${platform}/${mongodb}/${collection}.bson
            if [ $? == 0 ];then
