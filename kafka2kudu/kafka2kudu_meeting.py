...
try:
  ...
  for message in consumer:
    ...
    reslist=res.replace('["','').replace('"]','').split(',')
    apptype=reslist[0]
    clientappid=reslist[1]
    devicetype=reslist[2]
    deviceinfo=reslist[3]
    version=reslist[4]
    url=reslist[5]
    if url.startswith('http'):
      ...
      try:
        ...
        with open(URL_ERROR_PATH, mode='a') as filename:
          filename.write(str(e)+'|'+res+'|'+str(int(t))+'\n')
        print(e)
